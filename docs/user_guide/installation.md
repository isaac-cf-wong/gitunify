...
We recommend using `uv` to manage virtual environments for installing `gitunify`.
...
The recommended way to install `gitunify` is from PyPI:

```bash
# Create a virtual environment (recommended with uv)
uv venv --python 3.10
source .venv/bin/activate  # On Windows: .venv\Scripts\activate
uv pip install gitunify
```
...
uv pip install gitunify[dev]

# Documentation dependencies
uv pip install gitunify[docs]

# All dependencies
uv pip install gitunify[dev,docs]
```
...
```bash
git clone git@github.com:isaac-cf-wong/gitunify.git
cd gitunify
# Create a virtual environment (recommended with uv)
uv venv --python 3.10
source .venv/bin/activate  # On Windows: .venv\Scripts\activate
...
Check that `gitunify` is installed correctly:
...
```bash
python -c "import gitunify; print(gitunify.__version__)"
```
...
2. Search existing [issues](https://github.com/isaac-cf-wong/gitunify/issues)
